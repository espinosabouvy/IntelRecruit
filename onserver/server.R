...
shinyServer(function(input, output, session) {
     ...
     output$menu.login <- renderMenu({
          sidebarMenu(
               ...
               textInput("s.usuario","Usuario:",placeholder = "Usuario: demo",value = "demo"),
     ...
     observeEvent(input$b.login,{
          ...
          output$p.embudo <- renderPlot({
               ...
               embudo <<- q.embudo(id_usuario = id_user, 
                                                  date.inicio = fechas)   
          ...
          output$tabla.bolsa.candidatos <- DT::renderDataTable({
               ...
               tabla.bolsa <<- q.bolsa.vacantes()%>%
                    mutate_if(is.character, as.factor)
               
               row.names(tabla.bolsa)<- paste0(seq.int(1, nrow(tabla.bolsa),1),"-",tabla.bolsa$id)
               #filtrar rechazados 
               if(!is.null(input$ver.rechazados)) if(input$ver.rechazados==F) {
                    tabla.bolsa <- tabla.bolsa%>%filter(id_proceso != 5)     
               }
               tabla.bolsa <- tabla.bolsa%>%select(-id, -id_proceso)
               
               datatable(data = tabla.bolsa,
                         rownames = F,
                         selection ='single', 
                         filter = "top",
                         autoHideNavigation = T,
               ...
               if(id== ""){  #nuevo registro
                    ...
                    qinsert <- paste0("INSERT INTO candidatos
                                      (nombre, direccion, cp, fecha_nacimiento, id_sexo, id_escolaridad,
                                        id_medio) ",
                                      "VALUES ('",
                                      toupper(input$Tnombre), "', '",toupper(input$Tdireccion), "', ", input$Tcp, ", ",
                                      gsub("-","", ymd(input$Tnacimiento)), ", ", id_sexo, ", ",
                                      id_escolaridad, ", ", id_medio, ")")
                    dbExecute(con,qinsert)
                    
                    #id de ultimo candidato agregado
                    id_candidato <- dbGetQuery(con,"SELECT id FROM candidatos ORDER BY id DESC LIMIT 1")
                    ...
                    qinsert <- paste0("INSERT INTO vacantes_following
                                     ...
                                     VALUES (",
                                     id_candidato$id, ",", id_vacante$id, ",", 1, ",", gsub("-","", ymd(fecha.hoy)),")")
          ...
          output$tabla.catalogos <- renderDataTable({
               ...
               con <- conectar()
               tabla <- dbGetQuery(con, paste0("SELECT * FROM ", nombre.tabla))%>%
                    tibble::column_to_rownames('id')%>%
     ...
     cargar_menus <- function(){ #si login correcto
          ...
          if(level == "super"){
               output$menu.logged <- renderMenu({
                    sidebarMenu(
                         menuItem("SEGUIMIENTO", tabName = "abc-registro", icon = icon("bullseye"),selected = F),
                         menuItem("KPIs", tabName = "kpisl", icon = icon("signal")),
                         menuItem("DASHBOARD", tabName = "bi", icon = icon("dashboard")),
